...
if removed_rows > 0 or removed_cols > 0:
    msg += (
        f" Removed {removed_rows} completely empty rows and"
        f" {removed_cols} completely empty columns."
    )
st.success(msg)
...
with c1:
    target_col = st.selectbox(
        ...
        help=(
            "If the selected column is not binary (0/1), you will be able"
            " to map which values correspond to target=1 and target=0."
        ),
    )
...
default_bad_labels: list[str] = []
default_good_labels: list[str] = []
...
if conflict:
    st.sidebar.error(
        f"The following values are assigned to both 1 and 0: {conflict}. "
        "Please fix this."
    )
    st.stop()
...
if df_full["target"].nunique() < 2:
    st.sidebar.error(
        "Target has only one class after mapping. "
        "Please assign values to both 1 and 0."
    )
    st.stop()

st.sidebar.success(
    f"Mapped {len(bad_vals)} values to target=1 (BAD) and {len(good_vals)} values to "
    "target=0 (GOOD). Unselected values will be ignored."
)
...
max_missing_pct = st.slider(
    ...
    step=5,
    help=(
        "Columns with a higher percentage of missing values will be excluded"
        " from modeling."
    ),
)
...
with st.sidebar:
    ...
    if "issue_year" in df_full.columns and df_full["issue_year"].notna().any():
        years = (
            pd.to_numeric(df_full["issue_year"], errors="coerce")
            .dropna()
            .astype(int)
        )
    ...
    if "grade" in df_full.columns:
        opts = sorted(
            pd.Series(df_full["grade"]).dropna().astype(str).unique().tolist()
        )
    ...
    if "term" in df_full.columns:
        term_opts = (
            pd.Series(df_full["term"]).dropna().astype(str).unique().tolist()
        )
...
with k3:
    st.markdown(
        f"""
        <div class="kpi">
          <div class="label">Positive class rate (target = 1)</div>
          <div class="value">{pos_ratio}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
st.markdown(
    ...
    <div style="background:#f1f5f9;padding:10px 14px;border-radius:10px;
      ...
      This dashboard uses a binary target where
      <b>1 = BAD outcome (event)</b> and
...
if missing:
    st.warning(
        "These requested variables are absent or non-numeric in the filtered data: "
        + ", ".join(missing)
    )


# Helper for ranking (used by heatmap/pairwise defaults)
def get_featured_vars(df_in: pd.DataFrame, k: int = 6) -> list[str]:
    numeric_pool = [
        c for c in df_in.select_dtypes(include=[np.number]).columns if c != "target"
    ]

    target_num = None
    if "target" in df_in.columns:
        t = pd.to_numeric(df_in["target"], errors="coerce")
        if t.notna().sum() >= 2 and t.nunique(dropna=True) >= 2:
            target_num = t

    if target_num is not None and numeric_pool:
        tmp = pd.concat(
            [target_num.rename("target_num"), df_in[numeric_pool]], axis=1
        ).dropna()
        if not tmp.empty:
            cabs = (
                tmp.corr(numeric_only=True)["target_num"]
                .drop("target_num", errors="ignore")
                .abs()
            )
...
def categorical_cols(
    df_in: pd.DataFrame,
    max_card: int = 30,
    include_target_if_cat: bool = True,
) -> list[str]:
    """
    Return small-cardinality categorical-like columns
    (including low-cardinality numerics).
    """
    cats: list[str] = []

    for c in df_in.columns:
        if df_in[c].dtype.name in ("object", "category"):
            if df_in[c].dropna().nunique() <= max_card:
                cats.append(c)
        elif is_numeric_dtype(df_in[c]):
            u = df_in[c].dropna().nunique()
            if 2 <= u <= max_card:
                cats.append(c)

    if include_target_if_cat and "target" in df_in.columns:
        t = df_in["target"]
        if (t.dtype.name in ("object", "category")) or (
            is_numeric_dtype(t) and t.dropna().nunique() <= max_card
        ):
...
with tab_data:
    ...
    SAMPLE_N = EDA_SAMPLE_N if "EDA_SAMPLE_N" in globals() else 10_000
    ...
    for col in desc.columns:
        try:
            desc[col] = (
                pd.to_numeric(desc[col], errors="coerce")
                .round(3)
                .combine_first(desc[col])
            )
        except Exception:
            pass
...
with tab_dist:
    ...
    st.caption("🎯 Target legend: 0 = GOOD, 1 = BAD (as configured in sidebar)")
    ...
    if not EDA_VARS:
        ...
        src_all = df_eda[
            EDA_VARS + (["target"] if "target" in df_eda.columns else [])
        ].dropna()
        ...
        if show_line:
            ...
            y_var = st.selectbox(
                ...
                options=EDA_VARS,
                index=(
                    EDA_VARS.index("loan_amnt")
                    if "loan_amnt" in EDA_VARS
                    else 0
                ),
                key="line_y_var",
            )
            ...
            if not time_col:
                ...
                if time_col == "issue_year":
                    df_line["__time__"] = pd.to_numeric(
                        df_line[time_col], errors="coerce"
                    )
                else:
                ...
                line = (
                    ...
                    .encode(
                        ...
                        color=enc_color,
                        tooltip=[
                            alt.Tooltip("__time__:O", title="Time"),
        ...
        if show_hist:
            if not HIST_BOX_VARS:
                ...
                h_var = st.selectbox(
                    ...
                    index=(
                        HIST_BOX_VARS.index("loan_amnt")
                        if "loan_amnt" in HIST_BOX_VARS
                        else 0
                    ),
                    key="hist_var_single",
                )
                h_src = df_eda[
                    [h_var] + (["target"] if "target" in df_eda.columns else [])
                ].dropna()
                ...
                if "target" in h_src.columns:
                    hist = (
                        ...
                        .encode(
                            ...
                            tooltip=[
                                alt.Tooltip(f"{h_var}:Q", title=h_var),
                                "count()",
        ...
        if show_box:
            if "target" not in df_eda.columns:
                ...
                b_var = st.selectbox(
                    ...
                    options=HIST_BOX_VARS,
                    index=(
                        HIST_BOX_VARS.index("annual_inc")
                        if "annual_inc" in HIST_BOX_VARS
                        else 0
                    ),
                    key="box_var_single",
                )
...
with tab_corr:
    ...
    if num_df.empty or num_df.shape[1] < 2:
        ...
        if len(ordered) < 2:
            ordered = get_featured_vars(
                num_df, k=min(8, num_df.shape[1])
            )
...
with tab_ttest:
    ...
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.subheader("Welch’s t-tests (0 = GOOD, 1 = BAD)")
        ...
        if mask_valid.sum() < 2 or tnum[mask_valid].nunique() < 2:
            ...
            if not VARS:
                ...
                apply_fdr = st.checkbox(
                    "Apply FDR correction (Benjamini–Hochberg)", value=True
                )
                ...
                for col in VARS:
                    ...
                    df_welch = (se2**2) / (
                        ((v0 / len(g0)) ** 2) / (len(g0) - 1)
                        + ((v1 / len(g1)) ** 2) / (len(g1) - 1)
                    )
                ...
                if not rows:
                    ...
                    st.caption(
                        "Welch’s t-test (unequal variances). CI = 95% for mean "
                        "difference (1−0). Effect size = Cohen’s d (Hedges’ g corrected)."
                    )
...
with tab_cv:
    ...
    st.caption("Target legend — 0: GOOD, 1: BAD (event)")
    ...
    if "target" not in df.columns:
        ...
        if len(numeric_pool) == 0:
            ...
            default_pool = [
                c
                for c in ["int_rate", "dti", "revol_util", "loan_amnt", "annual_inc"]
                if c in numeric_pool
            ] or numeric_pool[:8]
            ...
            with st.expander("⚙️ CV settings (features & regularization)", expanded=False):
                ...
                balance_cv = st.checkbox(
                    "Class weight = 'balanced'", value=True, key="cv_bal"
                )
                top_k_cv = st.slider(
                    "Auto-select top-k features (by |coef|)",
                    3,
                    min(12, len(default_pool)),
                    6,
                    key="cv_topk",
                )
            ...
            try:
                ...
                y0_cv = (
                    pd.to_numeric(dtrain0_cv["target"], errors="coerce")
                    .astype(int)
                    .values
                )
            ...
            if run_cv:
                ...
                data_cv = df[["target"] + feats_cv].dropna().copy()
                X_all = data_cv[feats_cv].values
                y_all = (
                    pd.to_numeric(data_cv["target"], errors="coerce")
                    .astype(int)
                    .values
                )
                ...
                for fold_id, (tr_idx, va_idx) in enumerate(
                    ...
                    va_f1 = f1_score(
                        y_va, y_va_hat, average="binary", zero_division=0
                    )
                    va_auc = roc_auc_score(y_va, p_va)

                    cm = confusion_matrix(y_va, y_va_hat, labels=[0, 1])
                    rep = classification_report(
                        y_va, y_va_hat, digits=3, zero_division=0
                    )

                    cms.append(cm)
                    reports.append((fold_id, rep))
                    rows_cv.append(
                        {
                            "fold": fold_id,
                ...
                results_df = pd.DataFrame(rows_cv)
                ...
                st.caption(
                    "Threshold is optimized per fold to maximize F1 for the positive "
                    "class (1 = BAD / event)."
                )
    ...
    if "target" not in df.columns:
        ...
        if len(numeric_pool) == 0:
            ...
            try:
                ...
                y0 = (
                    pd.to_numeric(dtrain0["target"], errors="coerce")
                    .astype(int)
                    .values
                )
                ...
                y = (
                    pd.to_numeric(dtrain["target"], errors="coerce")
                    .astype(int)
                    .values
                )
            ...
            if visual == "Odds-ratio forest":
                ...
                coef_df = (
                    pd.DataFrame({"feature": feats, "odds_ratio": odds})
                ...
                base_chart = alt.Chart(coef_df).encode(
                ...
                if "ci_low" in coef_df.columns:
                    chart = bars + base_chart.mark_rule().encode(
                        x="ci_low:Q",
                        x2="ci_high:Q",
                        tooltip=[
                            "feature",
                ...
                st.markdown(
                    ...
                    "> 1 increases odds of target=1 (BAD); < 1 decreases."
                )
                ...
                plot_df["bin"] = pd.cut(
                    plot_df[one_x], bins=bins, include_lowest=True
                )
                line_df = (
                ...
                if len(feats) < 2:
                    ...
                    grid = (
                        tmp.groupby([bx, by], observed=False)["p1"]
                        .mean()
                        .reset_index()
                    )
                ...
                st.caption(
                    "Exploratory interpretation only • Standardized features • "
